...
**Console** (`web/`, Next.js 15, Vercel root dir `web`, env `API_BASE`): `/` inbox (`GET /reports?prefix=email_`, ISR 15 s), `/emails/[id]` diff board with the raw → normalised popover + "Show normalisation" switch (localStorage), `/queues` exception vs incomplete (client polling `/api/reports?status=NEEDS_REVIEW`), `/eval` from `web/public/evals/` (`npm run sync-evals` copies from `evals/`; the PNGs are git-ignored at the source but committed under `web/public`). No UI library; tokens identical to `api/index.html`. Dataset loaded into Firestore once with `scripts_load_cloud.py` (520 rows; category totals 220/60/75/125/40; MISMATCH 46, NEEDS_REVIEW 20).
